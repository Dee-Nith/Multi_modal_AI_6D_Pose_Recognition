-- Auto Kinect RGB + Depth Capture with Smart Increment
-- Captures both RGB and depth images from Kinect

print("Auto Kinect RGB + Depth Capture")
print("================================")
...
if kinectRGB ~= -1 and kinectDepth ~= -1 then
    ...
    function captureRGBImage(imageNumber)
        ...
        if rgbImage then
            ...
            for i, filename in ipairs(saveLocations) do
                print("Trying to save RGB to: " .. filename)
            ...
            print("Failed to save RGB to any location")
            return false
        else
    ...
    function captureDepthImage(imageNumber)
        ...
        if depthImage then
            ...
            -- Convert depth data to string
            local depthData = ""
            if type(depthImage) == "table" then
                -- Convert depth values to string representation
                for i, depth in ipairs(depthImage) do
                    if i > 1 then
                        depthData = depthData .. ","
                    end
                    -- Format depth value properly
                    depthData = depthData .. string.format("%.6f", depth)
                end
                print("Converted depth table to string: " .. #depthData .. " characters")
            else
                depthData = tostring(depthImage)
            end
            
            -- Try multiple save locations with incrementing number
            local saveLocations = {
                "auto_kinect_" .. imageNumber .. "_depth.txt",
                "/tmp/auto_kinect_" .. imageNumber .. "_depth.txt",
                "kinect_auto_" .. imageNumber .. "_depth.txt"
            }
            
            for i, filename in ipairs(saveLocations) do
                print("Trying to save depth to: " .. filename)
                local file = io.open(filename, "w")
                if file then
                    file:write(depthData)
                    file:close()
            ...
            print("Failed to save depth to any location")
            return false
        else
    ...
    if success then
        ...
        print("Files created:")
        print("  - auto_kinect_" .. nextNumber .. "_rgb.txt")
        print("  - auto_kinect_" .. nextNumber .. "_depth.txt")
