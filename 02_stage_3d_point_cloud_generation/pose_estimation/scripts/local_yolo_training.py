#!/usr/bin/env python3
"""
Local YOLOv8s Instance Segmentation Training Script
Trained on Roboflow dataset with 6 YCB object classes
"""
...
def prepare_dataset_paths():
    ...
    # Base dataset path - use the nested structure
    base_path = current_dir / "semantic_segmentation_project/Robot_Grasping_Dataset"
...
def create_training_config(base_path):
    ...
    # Save config in current directory to avoid path issues
    config_path = Path.cwd() / "data_local.yaml"
...
def train_yolov8s_instance_segmentation(config_path, device):
    ...
    # Training parameters
    training_args = {
        'data': str(config_path),  # Use the config path passed to this function
        'epochs': 100,
        'imgsz': 640,
        'batch': 8,
        'device': device,
        ...
        'cache': False,
        'workers': 4,
        'project': 'local_training_results',
...
def main():
    ...
    print("🎯 YOLOv8s Instance Segmentation Training")
    print("=" * 50)
    ...
    if model is not None:
        ...
        print(f"📁 Model saved in: local_training_results/yolov8s_instance_segmentation/")
        print(f"📊 Training results: {results.save_dir if results else 'N/A'}")
        
        # Save model path for later use
        model_path = Path("local_training_results/yolov8s_instance_segmentation/weights/best.pt")
