#!/usr/bin/env python3
"""
MCC-Final 3D Reconstruction Pipeline
Final optimized version with perfect parameters for CoppeliaSim data
"""
...
class MCCFinal3DReconstruction:
    def __init__(self, model_path="training_results/yolov8s_instance_segmentation/weights/best.pt"):
        """Initialize the MCC-final 3D reconstruction pipeline"""
        print("🚀 Initializing MCC-Final 3D Reconstruction Pipeline")
        
        # Load trained YOLO model
        self.model = YOLO(model_path)
        print(f"✅ YOLO model loaded: {model_path}")
        
        # 3D reconstruction parameters - OPTIMIZED for CoppeliaSim
        self.voxel_size = 0.05  # 5cm voxels (much larger for CoppeliaSim scale)
        self.depth_scale = 1000.0  # Depth scale factor
        self.min_points = 100  # Minimum points for reconstruction
        
        print("🎯 Pipeline ready for final 3D reconstruction!")
    ...
    def _apply_mcc_enhancements(self, pcd, detections):
        ...
        # 2. Downsample for efficiency (conservative)
    ...
    def _intelligent_object_filtering(self, pcd, detections):
        ...
        for detection in detections:
            ...
            if distance_from_center < 100:  # Close to center
                scale_factor = 1.2  # Less aggressive filtering
            elif distance_from_center < 200:  # Medium distance
                scale_factor = 1.0
            else:  # Far from center
                scale_factor = 0.8  # Even less aggressive filtering
            ...
            z_min, z_max = 0.01, 5.0  # Very wide depth range
        ...
        if np.sum(valid_mask) < len(points) * 0.2:  # If less than 20% points remain
            ...
            for detection in detections:
                ...
                width = x2 - x1
                height = y2 - y1
                x1_expanded = max(0, x1 - width * 0.5)
                x2_expanded = min(640, x2 + width * 0.5)
                y1_expanded = max(0, y1 - height * 0.5)
                y2_expanded = min(480, y2 + height * 0.5)
        ...
        if np.sum(valid_mask) < len(points) * 0.1:
            ...
            for detection in detections:
                ...
                x_min = max(0, x1 - 200) / 1000
                x_max = min(640, x2 + 200) / 1000
                y_min = max(0, y1 - 200) / 1000
                y_max = min(480, y2 + 200) / 1000
                z_min, z_max = 0.001, 20.0
    ...
    def _remove_ground_plane(self, pcd):
        ...
        try:
            ...
            plane_model, inliers = pcd.segment_plane(
                distance_threshold=0.05, ransac_n=10, num_iterations=300
            )
    ...
    def reconstruct_3d_objects(self, pcd, detections):
        ...
        for i, detection in enumerate(detections):
            ...
            # Create bounding box filter with very generous margins
            points = np.asarray(pcd.points)
    ...
    def run_full_pipeline(self, rgb_path, depth_path, output_path="mcc_final_3d_reconstruction_results"):
        """Run the complete MCC-final 3D reconstruction pipeline"""
        print("🚀 Starting MCC-Final 3D Reconstruction Pipeline")
        print("=" * 60)
    ...
    def _save_object_info(self, objects_3d, output_dir):
        ...
        with open(output_dir / "objects_3d_summary.txt", 'w') as f:
            f.write("MCC-Final 3D Reconstruction Results\n")
            f.write("=" * 50 + "\n\n")
...
def main():
    ...
    print("🎯 MCC-Final 3D Reconstruction Demo")
    print("=" * 50)
    
    # Initialize pipeline
    pipeline = MCCFinal3DReconstruction()
